...
async function KoloPodpowiedzWithFocus(event: Event) {
  event.preventDefault();
  await nextTick();
  storeFocus.ifPodpowiedzInFocus = true;
  await nextTick();
  if (!storeKola.ifZablokowaneKola) {
    storeKola.PokazPodpowiedz();
  }
...
<template>
  <div class="tlo">
    <div class="kola-elementy" v-if="storeKola.ifButtonPodpowiedz">
      <div
        class="elipsa elipsa-podpowiedz my-button"
        @click="KoloPodpowiedz"
        @keydown.enter="KoloPodpowiedzWithFocus"
        tabindex="0"
        aria-label="koło ratunkowe - podpowiedź"
      >
        <img
